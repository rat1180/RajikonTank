...
public class NPCSensor_Sight : NPCSensor_Base {
	...
	void GetTargetInSight(){
		...
		for (int i=0; i<overlapedObjects.Length; i++) {
			...
			/*	if (objAngle < SIGHT_INDIRECT_ANGLE && TargetInSight (overlapedObjects [points].transform, SIGHT_INDIRECT_DISTANCE / 2.0f)) {
					if(!somethingSpotted){
					...
					_lastTargetPos = overlapedObjects [points].transform.position;
				...
				if (alerted && objAngle < SIGHT_DIRECT_ANGLE && TargetInSight (overlapedObjects [points].transform, SIGHT_DIRECT_DISTANCE / 2.0f)) {																							
